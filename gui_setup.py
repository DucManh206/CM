...
import os # Cần cho os.path.basename trong select_script_file

# Import từ utils nếu cần (ví dụ: LOADED_USER_AGENT_LIST để kiểm tra bật/tắt checkbox)
from utils import LOADED_USER_AGENT_LIST

# --- Tab 1: Tạo Profile ---
def setup_create_tab(tab, app): # Nhận instance của app để truy cập/gán widget và gọi method
    ...
    # Gán widget vào instance của app
    ...
    entry_dir_create = customtkinter.CTkEntry(config_frame, textvariable=app.entry_dir_var, state="readonly") # Dùng biến chung
    entry_dir_create.grid(row=1, column=1, padx=10, pady=5, sticky="ew")
    # Gọi method của app instance khi nhấn nút
    ...
    if not LOADED_USER_AGENT_LIST: # Sử dụng biến đã import từ utils
    ...
    check_random_ua = customtkinter.CTkCheckBox(config_frame,
                                              ...
                                              variable=app.check_random_ua_var, # Gán biến vào app instance
                                              state=ua_checkbox_state)
    check_random_ua.grid(row=2, column=0, columnspan=3, padx=10, pady=(5,10), sticky="w")

    app.create_button = customtkinter.CTkButton(content_frame, text="Bắt đầu tạo Profiles", command=app.start_creation_thread) # Gọi method của app
    ...
    app.status_textbox_create = customtkinter.CTkTextbox(content_frame) # Gán status box
...
# --- Tab 2: Quản lý ---
def setup_manage_tab(tab, app): # Nhận instance của app
    ...
    entry_display_dir = customtkinter.CTkEntry(top_frame, textvariable=app.entry_dir_var, state="readonly") # Dùng biến chung
    entry_display_dir.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
    button_browse_manage = customtkinter.CTkButton(top_frame, text="Chọn thư mục", width=100, command=app.browse_directory) # Gọi method của app
    button_browse_manage.grid(row=0, column=2, padx=(0,5), pady=5)
    button_refresh_manage = customtkinter.CTkButton(top_frame, text="Làm mới DS", width=100, command=app.refresh_profile_list_manage) # Gọi method của app
    button_refresh_manage.grid(row=0, column=3, padx=5, pady=5)

    list_container_frame = customtkinter.CTkFrame(tab)
    list_container_frame.grid(row=1, column=0, padx=10, pady=(5,5), sticky="nsew")
    ...
    # Gán frame list vào app instance
    app.profile_list_frame_manage = customtkinter.CTkScrollableFrame(list_container_frame)
    app.profile_list_frame_manage.grid(row=1, column=0, padx=0, pady=(0,5), sticky="nsew")

    action_frame = customtkinter.CTkFrame(tab)
    action_frame.grid(row=2, column=0, padx=10, pady=(0,5), sticky="ew")
    action_frame.columnconfigure(0, weight=1)
    action_frame.columnconfigure(1, weight=1)
    button_open_selected = customtkinter.CTkButton(action_frame, text="Mở đã chọn", command=app.open_selected_profiles_manage) # Gọi method của app
    button_open_selected.grid(row=0, column=0, padx=5, pady=5, sticky="ew")
    button_delete_selected = customtkinter.CTkButton(action_frame, text="Xóa đã chọn", fg_color="red", hover_color="darkred", command=app.delete_selected_profiles_manage) # Gọi method của app
    button_delete_selected.grid(row=0, column=1, padx=5, pady=5, sticky="ew")

    app.status_textbox_manage = customtkinter.CTkTextbox(tab, height=80) # Gán status box
...
# --- Tab 3: Script ---
def setup_script_tab(tab, app): # Nhận instance của app
    """Thiết lập các widget cho tab Script."""
    tab.columnconfigure(0, weight=1)
    tab.rowconfigure(1, weight=1) # Cho list frame giãn
    tab.rowconfigure(3, weight=1) # Cho ô paste script giãn
    tab.rowconfigure(5, weight=0) # Status box co lại
    ...
    entry_display_dir_script = customtkinter.CTkEntry(top_frame_script, textvariable=app.entry_dir_var, state="readonly") # Dùng biến chung
    entry_display_dir_script.grid(row=0, column=1, padx=5, pady=5, sticky="ew")
    button_refresh_script = customtkinter.CTkButton(top_frame_script, text="Làm mới DS", width=100, command=app.refresh_profile_list_script) # Gọi method của app
    ...
    script_runner_frame.grid(row=2, column=0, padx=10, pady=5, sticky="ew")
    ...
    button_select_script = customtkinter.CTkButton(select_file_frame, text="1. Chọn File Script (.py)", width=180, command=app.select_script_file) # Gọi method của app
    button_select_script.grid(row=0, column=0, padx=5, pady=5, sticky="w")
    # Gán label hiển thị tên file script
    ...
    # Gán nút chạy script
    app.run_script_button = customtkinter.CTkButton(script_runner_frame, text="2. Chạy Script trên Profile đã chọn", command=app.start_script_runner_thread) # Gọi method của app
    app.run_script_button.grid(row=3, column=0, padx=5, pady=10, sticky="ew")

    # Gán status box của tab script
